...
export async function registerRoutes(
  ...
  // Profile routes
  app.get("/api/profile", authMiddleware as RequestHandler, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId;
      const user = await storage.getUser(userId);
  ...
  app.put("/api/profile", authMiddleware as RequestHandler, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId;
      ...
      console.error("Update profile error:", error);
      require('fs').writeFileSync('profile-error.log', String(error) + '\n' + (error.stack || ''));
      res.status(500).json({ message: "Failed to update profile" });
    }
  });

  app.patch("/api/profile/photo", authMiddleware as RequestHandler, upload.single("photo"), async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId;
      ...
      if (process.env.IMAGEKIT_PUBLIC_KEY && process.env.IMAGEKIT_PRIVATE_KEY && process.env.IMAGEKIT_URL_ENDPOINT) {
        ...
        console.warn("ImageKit keys missing, using mock upload.");
        imageUrl = "https://imagekit.io/dashboard/media-library/L2ltYWdlcw" + userId + "&t=" + Date.now(); 
      }
      
      const user = await storage.updateUser(userId, { profileImageUrl: imageUrl });
      if (!user) return res.status(404).json({ message: "User not found" });
