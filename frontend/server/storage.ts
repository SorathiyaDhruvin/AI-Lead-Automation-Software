import { db } from "./db";
import { usersLegacy, leads, segments, activities, leadNotes, leadRequests, automationRules, notifications, passwordResets } from "@shared/schema";
import { eq, desc, gte, lte, ilike, and, or, sql, type SQL } from "drizzle-orm";
import type { InsertUserLegacy, UserLegacy, InsertLead, Lead, InsertSegment, Segment, InsertActivity, Activity, InsertLeadNote, LeadNote, InsertLeadRequest, LeadRequest, InsertAutomationRule, AutomationRule, InsertNotification, Notification, InsertPasswordReset, PasswordReset } from "@shared/schema";
...
export interface IStorage {
  ...
  getPasswordResetByEmailAndOtp(email: string, otp: string): Promise<PasswordReset | undefined>;
  getPasswordResetByToken(token: string): Promise<PasswordReset | undefined>;
  updatePasswordReset(id: string, updates: Partial<PasswordReset>): Promise<PasswordReset | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Password reset methods
  async createPasswordReset(insertData: InsertPasswordReset): Promise<PasswordReset> {
    const [reset] = await db.insert(passwordResets).values(insertData).returning();
    return reset;
  }

  async getPasswordResetByEmailAndOtp(email: string, otp: string): Promise<PasswordReset | undefined> {
    const [reset] = await db.select().from(passwordResets).where(and(eq(passwordResets.email, email), eq(passwordResets.otp, otp)));
    return reset;
  }
  ...
  async updatePasswordReset(id: string, updates: Partial<PasswordReset>): Promise<PasswordReset | undefined> {
    const [reset] = await db.update(passwordResets).set(updates).where(eq(passwordResets.id, id)).returning();
    return reset;
  }
  ...
  async getNotesByLead(leadId: string): Promise<Array<LeadNote & { authorName: string }>> {
    const rows = await db
      .select({
        ...
        authorName: sql<string>`concat(${usersLegacy.firstName}, ' ', ${usersLegacy.lastName})`,
      })
      .from(leadNotes)
