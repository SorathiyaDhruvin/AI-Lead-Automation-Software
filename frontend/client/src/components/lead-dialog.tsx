...
export function LeadDialog({ open, onOpenChange, lead }: LeadDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: LeadFormValues) => leadsService.create(data),
    onSuccess: () => {
      // Invalidate all lead-related queries (list, filters, sub-queries)
      queryClient.invalidateQueries({ queryKey: ["/api/leads"] });
      // Refresh dashboard statistics, charts, and AI analytics
      queryClient.invalidateQueries({ queryKey: ["/api/dashboard/stats"] });
      // Refresh segments (lead counts may change)
      queryClient.invalidateQueries({ queryKey: ["/api/segments"] });
      // Refresh notification bell
      queryClient.invalidateQueries({ queryKey: ["/api/notifications"] });
      toast({ title: "Lead Created Successfully", description: "New lead has been added to your pipeline" });
      form.reset();
      onOpenChange(false);
    },
    onError: (error: Error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: LeadFormValues) => {
      ...
      return leadsService.update(lead.id, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/leads"] });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button type="submit" disabled={isPending} data-testid="button-submit-lead">
                ...
                {isPending ? (isEditing ? "Updating..." : "Creating...") : isEditing ? "Update Lead" : "Create Lead"}
              </Button>
            </div>
          </form>
        </Form>
