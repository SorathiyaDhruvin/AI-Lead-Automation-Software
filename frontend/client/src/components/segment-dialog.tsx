...
export function SegmentDialog({ open, onOpenChange, segment }: SegmentDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: SegmentFormValues) => segmentsService.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: SegmentFormValues) => {
      if (!segment?.id) throw new Error("No segment selected");
      return segmentsService.update(segment.id, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/segments"] });
