...
export function DashboardPage() {
  ...
  const { data: leads, isLoading: leadsLoading } = useQuery<Lead[]>({
    ...
    queryFn: () => leadsService.getAll(undefined, 5),
  });
  ...
  const avatarUrl = userProfile?.profile_image_url || user?.user_metadata?.avatar_url || user?.user_metadata?.picture;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Welcome back, <span className="font-semibold text-foreground">{displayName}</span>! Here's your lead automation overview.
          </p>
        </div>
